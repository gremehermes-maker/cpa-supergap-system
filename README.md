...
## 🤖 [AI AGENT ABSOLUTE RULE] 자동 형상 관리 및 백업 규칙
**이 저장소에서 작업하는 모든 AI 에이전트(Antigravity, Hermes 등)는 사용자가 신경 쓰지 않도록 아래 규칙을 무조건 준수하여 자동으로 코드를 동기화할 것.**
1. **작업 시작(Wake up) 시:** 코드를 수정하기 전, 최우선적으로 `git pull origin main`을 실행하여 윈도우/맥 환경의 최신 버전을 덮어쓸 것.
2. **작업 종료(Sleep) 전:** 코딩이나 수정이 완료되면 사용자가 명령하지 않더라도 작업 마무리 단계에서 항상 `git add .`, `git commit -m "자동 백업: 작업 내용 요약"`, `git push origin main`을 실행하여 GitHub 원격 저장소에 백업할 것.

### 📝 [AI AGENT SOP] 모듈 신규 개발 및 수정 시 문서화 표준 지침
**사용자(Human)가 매번 지시하느라 시간을 낭비하지 않도록, 에이전트가 코드를 건드리면 반드시 아래 문서를 스스로 업데이트할 것.**
1. **모듈 명세서 업데이트:** `docs/module_specs/` 폴더 내의 해당 모듈 마크다운 문서(`.md`)를 찾아 변경된 작동 원리, 프롬프트, 입출력 스펙을 상세히 기록할 것.
2. **코드 일치성 확보:** 파이썬 코드(`src/` 하위)의 내용과 `module_specs/`의 내용이 완벽하게 일치하는지 스스로 교차 검증할 것.
3. **README 업데이트:** 만약 새로운 모듈(파일)이 생성되었다면, 본 `README.md` 파일 하단의 **[시스템 모듈별 참조 문서]** 목록에 반드시 새 문서의 링크를 추가할 것.
